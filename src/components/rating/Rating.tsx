...
const Rating = () => {
  ...
  const hasUserRated = useSessionStore((s) => s.hasUserRated);
  ...
  const checkMark = () => {
    return (
      <svg
        style={{ width: "120px", color: "lightgreen", marginTop: "80px" }}
  ...
  const thankYouPage = () => {
    return (
      <div className="thankyoupage" style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "650px", flexDirection: "column" }}>
        ...
        <button style={{ margin: "0", width: "100px" }} type="button" onClick={() => setModalOpen(false)}>
          Close
        </button>
      </div>
    );
    ...
    <div className="rating">
      <div className="ratingdiv">
        <Modal isOpen={modalOpen} onRequestClose={() => setModalOpen(false)} style={customStyles} shouldCloseOnOverlayClick={false}>
          {hasUserRated ? thankYouPage() : ratingOptions()}
        </Modal>
      </div>
    </div>
  );
