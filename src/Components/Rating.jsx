...
const Rating = () => {
  ...
  const questionMark = () => {
    return (
      <button
        onClick={() => { ratingInfo.setOpenHowTo(true) }}
  ...
  const checkMark = () => {
    return (
      <svg style={{ width: '120px', color: 'lightgreen', marginTop: '80px' }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
  ...
  const handleRatingClick = (rating) => {
    data = data + rating;
    // console.log(data);
    ratingInfo.setHasUserRated(true);
    localStorage.setItem(ratingInfo.blockNum, data);
    // console.log(localStorage.getItem(ratingInfo.blockNum));
  };
  ...
  const thankYouPage = () => {
    return (
      <div className="thankyoupage" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '650px', flexDirection: "column" }}>
        {checkMark()}
